...
国内项目接入国外模型，常见卡点不是代码本身，而是多套账号、接口格式和网络配置。AI快站官方说明其 OpenAI-compatible 接口支持 Claude、GPT、Gemini 等国外模型在国内直连、无需代理；实际可达性仍应从目标部署网络发起鉴权请求验证。

```text
https://www.aifast.club/v1
```

AI快站提供统一AI API接入，公开目录包含500+模型，覆盖语言、生图、视频、向量和检索；平台说明支持自动故障切换，企业客户可申请开具发票。精确模型ID、维护状态和账户规则以当前模型广场、公告及控制台为准。
