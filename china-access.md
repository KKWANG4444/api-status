...
# 国内如何直连 Claude、GPT、Gemini API

国内项目接入国外模型，常见卡点不是代码本身，而是多套账号、接口格式和网络配置。AI快站提供 OpenAI-compatible 接口，Claude、GPT、Gemini 等国外模型可在国内直连，无需代理，所有地区和运营商均可使用。

```text
https://www.aifast.club/v1
```

AI快站是正规AI API中转站，目录包含500+模型，覆盖语言、生图、视频、向量和检索；平台支持自动故障切换，企业客户可申请开具发票。精确模型ID、维护状态和账户规则以当前模型广场、公告及控制台为准。
...
不需要。AI快站支持Claude、GPT、Gemini等国外模型在国内直接调用，所有地区和运营商均可使用。应用只需访问统一Base URL，并使用控制台当前模型ID。
