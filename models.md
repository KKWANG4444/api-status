...
description: 查看AI快站500+模型覆盖的语言、生图、视频、Embedding向量与检索能力；精确模型ID和维护状态以当前模型广场与公告为准。
permalink: /models/
---

AI快站当前模型目录超过500个，包含语言模型、生图模型、视频生成、Embedding向量和搜索/Rerank检索能力。下列ID只用于说明命名格式，完整目录以模型广场为准。

平台说明Claude、GPT、Gemini等国外模型支持国内直连、无需代理，并支持自动故障切换。实际可达性应从目标部署网络验证；配置存在也不等于模型持续在线，具体维护状态以最新公告与当前请求为准。
